...
def find_feedback_occurrences(transcripts):
    ...
    for file, utts in utts_child.items():
        ...
        utts_filtered = utts_filtered[(utts_filtered.speaker_code == SPEAKER_CODE_CHILD) &
        ...
        for candidate_id in utts_filtered.index.values:
            ...
            following_utts_child = following_utts[following_utts.speaker_code == SPEAKER_CODE_CHILD]
...
if __name__ == "__main__":
    ...
    file_name = os.path.expanduser(f"~/data/communicative_feedback/feedback{'_'.join(args.corpora)}.csv")
    ...
    feedback["utt_child_follow_up"] = feedback.utt_child_follow_up.apply(remove_babbling)
    ...
    for age in AGE_BINS:
        feedback_age = feedback[(feedback.age > age - AGE_BINS_WINDOW) & (feedback.age <= age + AGE_BINS_WINDOW)]
        print(f"\nFound {len(feedback_age)} turns for age {age} (+/- {AGE_BINS_WINDOW} months)")
        if len(feedback_age) > 0:
            mean_length_intelligible = feedback_age[feedback_age.intelligible].length.mean()
            print(f"Mean pause length after intellgible utts: {mean_length_intelligible:.3f}ms")
            mean_length_unintelligible = feedback_age[~feedback_age.intelligible].length.mean()
            print(f"Mean pause length after unintelligible utts: {mean_length_unintelligible:.3f}ms")

            # Caregiver contingency:
            n_responses_clear = len(feedback_age[(feedback_age.intelligible) & (feedback_age.length <= RESPONSE_THRESHOLD)])
            n_clear = len(feedback_age[feedback_age.intelligible])

            n_responses_unclear = len(feedback_age[~feedback_age.intelligible & (feedback_age.length <= 1)])
            n_unclear = len(feedback_age[~feedback_age.intelligible])

            contingency_caregiver = (n_responses_clear / n_clear) - (n_responses_unclear / n_unclear)
            ...
            n_intelligible_follow_up_if_response = len(feedback_age[feedback_age.intelligible_follow_up & feedback_age.intelligible & (feedback_age.length <= 1)])
            n_responses = len(feedback_age[feedback_age.length <= 1])

            n_intelligible_follow_up_if_no_response = len(feedback_age[feedback_age.intelligible_follow_up & feedback_age.intelligible & (feedback_age.length > 1)])
            n_no_respones = len(feedback_age[feedback_age.length > 1])

            if (n_responses > 0) and (n_no_respones > 0) and (n_intelligible_follow_up_if_no_response > 0):
                contingency_children = (n_intelligible_follow_up_if_response / n_responses) / (n_intelligible_follow_up_if_no_response / n_no_respones)
